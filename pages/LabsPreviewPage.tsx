...
/**
 * Order: Visual Audiobooks leads, by Jeanine's call (2026-08-14). The
 * earlier rule here was "never open the chapter on an unbuilt project,"
 * with Visual Audiobooks parked third until she shipped it; the finished
 * Living Photocopy cover film is that graduation, with a first prototype
 * in hand. Static, the previous opener, moves to second. AI Creator Lab
 * stays a Feature (promoted from the plan's default Short for carrying
 * the chapter's strongest verified adoption number) and stays fourth,
 * where the recruiter-persona critique wanted the "4 to 50+" stat kept
 * inside the chapter's front half rather than 24 viewport-heights deep.
 *
 ...
 * missing `flip` renders right. This ENTRIES array is the Feature run
 * only: four entries (Visual Audiobooks, Static, Multiverse Quad, AI
 * Creator Lab) running right/left/right/left — the 2026-08-14 reorder
 * moved the flip from Multiverse Quad to Static to keep that sequence.
 * The three demoted entries live in CREDIT_ENTRIES below and render as
 * one credits screen, where `flip` is not read at all.
 */
const ENTRIES: LabEntry[] = [
  {
    ...
    description: (
      <>
        ...
        <p className="mt-3">
          Each reading can reveal a different visual interpretation. Some
          editions are illustrated by artists. Others are generated by
          code.
        </p>
        <p className="mt-3">
    ...
    expandables: [
      ...
      },
      {
        ...
        //
        // "Launching soon." is the relocated badge, kept in its exact
        // approved wording rather than rephrased into a new claim.
        label: 'Build',
  ...
  },
  {
    ...
    // Gained in the 2026-08-14 reorder: second position is a left frame
    // now that Visual Audiobooks opens the run on a right one.
    flip: true,
  },
  {
    id: 'multiverse-quad',
    ...
    // Flip moved to Static in the 2026-08-14 reorder: third position is a
    // right frame again, as it was when this entry sat second with one.
  },
  {
    id: 'ai-creator-lab',
    client: 'Wondery',
 ...
 *
 * The Feature run is now four: Visual Audiobooks, Static, Multiverse Quad,
